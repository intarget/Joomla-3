...
function formCheck(){
    ...
    var errMsg = {'both':'Пожалуйста, введите email и ключ','key':'Пожалуйста, введите ключ','email':'Пожалуйста, введите email'};
    //jQuery('button#intrgt_auth_btn').attr('disabled','disabled');
//event.returnValue=false;formCheck(); return false;
    var formError = '';
    if ((jQuery('#jform_params_app_key').val() == '') && (jQuery('#jform_params_email').val() == '')) {
        formError = 'both';
        //jQuery('#jform_params_app_key').parent().parent().addClass('error');
        //jQuery('#jform_params_email').parent().parent().addClass('error');

    } else if (jQuery('#jform_params_app_key').val() == '') {
        //jQuery('#jform_params_app_key').parent().parent().addClass('error');
        formError = 'key';
    } else if (jQuery('#jform_params_email').val() == '') {
        //jQuery('#jform_params_email').parent().parent().addClass('error');
        formError = 'email';
    };
    if (formError == '') {
    ...
    if ((jQuery('#jform_params_app_key').val() !== '') && (jQuery('#jform_params_email').val() !== '')) {
        if (window.intarget_succes_reg == true) {
           ...
           //выключаем поля
            ...
            jQuery('[id=jform_params_email-lbl]').parent().parent().after(success_text+support_text);
            //jQuery('button#intrgt_auth_btn').hide();
        } else if (window.intarget_succes_reg == false) {
            jQuery('[id=jform_params_email-lbl]').parent().parent().after(text_after+support_text);
            if (window.intarget_reg_error == 403){
                ...
                jQuery('.alert.alert-success').hide();
                if (validateEmail(jQuery('#jform_params_email').val())){
                    var error_text = 'Данный email не зарегистрирован на сайте <a href="http://intarget.ru">http://intarget.ru</a>';
                } else {
                    var error_text = 'Ошибка! Проверьте правильность ввода email.';
                }
            ...
            var intrg_btn_html = '<div style="width:100%;margin-top: 20px;">'+
                ...
                '</div>'+
                '</div>';
            jQuery('input#jform_params_email').parent().after(intrg_btn_html)
        } else {
            jQuery('input#jform_params_email').parent().after('<button style="float:left;margin-top: 15px;" id="intrgt_auth_btn" onclick="formCheck(); return false;"> Авторизация </button>')
            jQuery('[id=jform_params_email-lbl]').parent().parent().after(text_after+support_text);};
