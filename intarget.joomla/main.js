    ...
    jQuery(document).ready(function () {
        ...
        if ((jQuery('#jform_params_app_key').val() !== '') && (jQuery('#jform_params_email').val() !== '')) {
            if (window.intarget_succes_reg == true && jQuery('#jform_params_intarget_id').val() !== '') {
                jQuery('#jform_params_app_key').after('<img title="Введен правильный Ключ API!" class="intrg_ok" src="../plugins/system/intarget/ok.png">');
                ...
                if (window.intarget_reg_error == 500) {
                    jQuery('.alert.alert-success').hide();
                    var error_text = 'Данный сайт уже используется в другом аккаунте на сайте <a href="http://intarget.ru" target="_blank">inTarget</a>';
                }
                if (window.intarget_reg_error == 404) {
                    jQuery('.alert.alert-success').hide();
                    var error_text = 'Данный email не зарегистрирован на сайте <a href="http://intarget.ru" target="_blank">inTarget</a>';
                }
